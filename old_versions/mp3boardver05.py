...
class AudioButtonApp:
    def __init__(self, root):
        ...
        # Estilo para botones de la matriz
        self.style = ttk.Style()
        self.style.configure(
            "Matrix.TButton",
            background="#0078D7",  # Azul
            foreground="white",
        ...
        self.style.map(
            "Matrix.TButton",
            background=[("active", "#005BB5")],  # Azul más oscuro al presionar
        )
    ...
    def create_buttons(self):
        ...
        for row in range(5):
            button_row = []
            for col in range(5):
                frame = tk.Frame(self.root, height=300, width=150)  # Tamaño de celda aumentado
                frame.grid_propagate(False)  # Desactivar redimensionamiento
                ...
                button.pack(expand=True, fill="both")  # Rellenar el frame
    ...
    def handle_button_press(self, row, col):
        ...
        if button_key in self.button_audio_map:
            ...
            if audio_file:
                ...
                self.buttons[row][col].config(style="Matrix.TButton")  # Cambiar estilo para indicar asociación
                self.save_config(DEFAULT_CONFIG_FILE)
        ...
        if os.path.exists(config_file):
            try:
                ...
                for button_key in self.button_audio_map.keys():
                    row, col = map(int, button_key.split(","))
                    self.buttons[row][col].config(style="Matrix.TButton")
    ...
    def reset_config(self):
        ...
        if messagebox.askyesno("Confirmar", "¿Está seguro de que desea resetear toda la configuración?"):
            ...
            for row in range(5):
                for col in range(5):
                    self.buttons[row][col].config(text=f"{row+1},{col+1}", style="Matrix.TButton")
            self.save_config(DEFAULT_CONFIG_FILE)
